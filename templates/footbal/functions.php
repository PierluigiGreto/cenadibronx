...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    function artxPost($data)
    {
    	...
    	artxFragmentBegin(str_replace('class="style-box style-post">', 'class="style-box style-post' . $classes . '">', "<div class=\"style-box style-post\">\r\n    <div class=\"style-box-body style-post-body\">\r\n<div class=\"style-post-inner\">\r\n"));
    	artxFragmentBegin("<h2 class=\"style-postheader\">");
    	artxFragmentBegin("");
    	...
    	artxFragmentEnd("\r\n");
    	artxFragmentEnd("</h2>\r\n");
    	artxFragmentBegin("<div class=\"style-postheadericons style-metadata-icons\">\r\n");
    	if (isset($data['metadata-header-icons']) && count($data['metadata-header-icons']))
    		foreach ($data['metadata-header-icons'] as $icon)
    			artxFragment('', $icon, '', ' | ');
    	artxFragmentEnd("\r\n</div>\r\n");
    	artxFragmentBegin("<div class=\"style-postcontent\">\r\n");
...
<div class="style-box style-block">
        ...
        <?php echo str_replace('class="style-box style-block">', 'class="style-box style-block' . $classes . '">', ob_get_clean()); ?>
        <?php if ($hasCaption): ?>
...
?><div class="style-box style-vmenublock">
    <div class="style-box-body style-vmenublock-body">
<?php

        echo str_replace('class="style-box style-vmenublock">', 'class="style-box style-vmenublock' . $classes . '">', ob_get_clean());
        if ($hasCaption):
?><div class="style-bar style-vmenublockheader">
    <h3 class="t"><?php
...
?></h3>
        ...
        if ($hasContent):
?><div class="style-box style-vmenublockcontent">
    <div class="style-box-body style-vmenublockcontent-body">
<?php
...
<?php
    ...
    function artxModules(&$document, $position, $style = null)
    {
         ...
         *  Select inputs with class != 'style-button'
         *   artxFindTags($html, array('input' => array('class' => '!style-button')))
         *  Select inputs with non-empty class
         *   artxFindTags($html, array('input' => array('class' => '!')))
         *  Select inputs with class != 'style-button' and non-empty class:
         *   artxFindTags($html, array('input' => array('class' => array('!style-button', '!'))))
         *  Select inputs with class = button but != 'style-button'
         *   artxFindTags($html, array('input' => array('class' => array('button', '!style-button'))))
         *
        ...
        function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
        {
            ...
            foreach (artxFindTags($content, $filter) as $tag) {
                if (is_string($tag))
                    ...
                    $tag['attributes']['class']['value'][] = 'style-button';
                    $delimeter = '' == $tag['attributes']['class']['delimeter']
                        ? '"' : $tag['attributes']['class']['delimeter'];
                    $tag['attributes']['class']['delimeter'] = $delimeter;
                    $button = str_replace('"', $delimeter, '<span class="style-button-wrapper">'
                        . '<span class="style-button-l"> </span><span class="style-button-r"> </span>')
        ...
        {
            return '<span class="style-button-wrapper"><span class="style-button-l"> </span><span class="style-button-r"> </span>'
                . '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
                . 'style-button" href="' . $data['link'] . '">' . $data['content'] . '</a></span>';
        }
...
function artxPostprocessBlockContent($content)
    ...
    {
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!style-button')),
            'button' => array('class' => array('button', '!style-button'))));
        return $content;
    }
