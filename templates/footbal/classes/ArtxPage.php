...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="style-content-layout">';
        $result .= '<div class="style-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="style-layout-cell' . ('style-block' == $style ? ' style-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="style-post">' string in it.
     * Then it wraps content of the buffer with style-post.
     */
    public function componentWrapper()
    {
        ...
        if ('com_content' == $option && ('featured' == $view || 'article' == $view || ('category' == $view && 'blog' == $layout)))
            return;
        if (false === strpos($content, '<div class="style-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'style-main').addClass('style-postcontent');
            }
        });
    }
});
